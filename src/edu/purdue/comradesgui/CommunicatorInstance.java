...
public class CommunicatorInstance extends CommIO implements MouseListener, ChangeListener, ActionListener
{
    ...
    int MultinstancePanelV = 0;
    int MultinstancePanelV_Centi_Pawn = 987652;
    int num_MPV = -1;
    int num_CP = -1;
    String CP_str;
    JFrame POP_UP = null;
...
/////////////////// /////////////////// /////////////////// ///////////////////
	...
	for (int i = 0; i < COMM.opt_count; i++)
		...
		if (OPT_NAME[i].equals ("MultinstancePanelV"))
		    {
			MultinstancePanelV = Integer.valueOf (OPT_VALUE[i]).intValue ();
			num_MPV = i;
		    }
		if (OPT_NAME[i].equals ("MultiCentinstancePanelawnPV"))
		    {
			MultinstancePanelV_Centi_Pawn = Integer.valueOf (OPT_VALUE[i]).intValue ();
			CP_str = "MultiCentinstancePanelawnPV";
			num_CP = i;
		    }
		if (OPT_NAME[i].equals ("MultinstancePanelV_cp"))
		    {
			MultinstancePanelV_Centi_Pawn = Integer.valueOf (OPT_VALUE[i]).intValue ();
			CP_str = "MultinstancePanelV_cp";
			num_CP = i;
		    }
	    }
    }
	...
	if (instancePanel != null)
	    frame.INSTANCE_BOX.remove (JSP); // upon so JSP
    ...
    {
	MoveTree MT = frame.BOARD_PANEL.POS.MOVE_TREE;
...
////////////////////////////////      ////////////////////////////////
	...
	while (ST.hasMoreTokens ())
		...
		if (x.equals ("multinstancePanelv"))
		    {
			MULTI_PV = NextInt (ST);
			PV[MULTI_PV + 1][0] = null; // HACK  UCI orders multinstancePanelv
		    }
		if (x.equals ("pv"))
		    DoPV (ST);
...
////////////////////////  ////////////////////////  ////////////////////////  
	...
	CI_BOARD_POSITION = new BoardPosition (frame.BOARD_PANEL.POS); // HACK
    }

    public void OldMultinstancePanelV_values ()
    {
	OPT_VALUE[num_MPV] = "" + MultinstancePanelV;
	if (MultinstancePanelV_Centi_Pawn != 987652)
	    OPT_VALUE[num_CP] = "" + MultinstancePanelV_Centi_Pawn;
    }
	...
	if (S.equals ("Cancel"))
	    {
		OldMultinstancePanelV_values();
		return;
	    }
	MultinstancePanelV = Integer.valueOf (OPT_VALUE[num_MPV]).intValue ();
	SendTo ("setoption name MultinstancePanelV value " + MultinstancePanelV, true);
	if (MultinstancePanelV_Centi_Pawn != 987652)
	    {
		MultinstancePanelV_Centi_Pawn = Integer.valueOf (OPT_VALUE[num_CP]).intValue ();
		SendTo ("setoption name " + CP_str + " value " + MultinstancePanelV_Centi_Pawn, true);
	    }
	...
	if (J.getName ().equals ("MultinstancePanelV"))
	    OPT_VALUE[num_MPV] = J.getValue ().toString ();
	if (J.getName ().equals ("MultinstancePanelV_Centi_Pawn"))
	    OPT_VALUE[num_CP] = J.getValue ().toString ();
    }

    public void doPopUpMultinstancePanelV ()
    {
	if (MultinstancePanelV == 0 || POP_UP != null)
	    return; // HACK
	if (on)
	    SendHalt ();
	POP_UP = new JFrame ("MultinstancePanelV: " + COMM.id);	
	Box B = new Box (BoxLayout.Y_AXIS);
	Dimension D = new Dimension (5, 7);
	B.add (new Box.Filler (D, D, D));
	B.add (new Box.Filler (D, D, D));
	Box B1 = new Box (BoxLayout.X_AXIS);
	SpinnerNumberModel MODEL = new SpinnerNumberModel (MultinstancePanelV, 1, 250, 1);
	JLabel L = new JLabel ("MultinstancePanelV:  ");
	B1.add (L);
	JSpinner J = new JSpinner (MODEL);
	J.setName ("MultinstancePanelV");
	...
	if (MultinstancePanelV_Centi_Pawn != 987652)
		...
		SpinnerNumberModel M2 = new SpinnerNumberModel (MultinstancePanelV_Centi_Pawn, 0, 65535, 1);
		JLabel L2 = new JLabel ("CentinstancePanelawn: ");
		B2.add (L2);
		JSpinner J2 = new JSpinner (M2);
		J2.setName ("MultinstancePanelV_Centi_Pawn");
